...
function package_chart() {
  ...
  # Vendor the dependencies only when they are missing: a build box that
  # already has them stays offline, and packaging does not turn a reachability
  # problem at somebody else's chart repository into a failed build.
  #
  # `dependency update`, not `build`: Chart.lock is not tracked, and `build`
  # refuses a repository that was never `helm repo add`-ed.
  if [[ -z "$(ls -A "${chart_path}/charts" 2>/dev/null)" ]]; then
